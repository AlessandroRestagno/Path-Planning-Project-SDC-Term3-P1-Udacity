...
int main() {
  ...
  */
  ...
  h.onMessage([&map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy, &lane, &ref_vel, &lane_changing, &move_two_lanes, &time_change, &two_lane_change_starting_lane](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			for (int i=0; i < sensor_fusion.size(); i++) {
				...
				if ((check_car_s > car_s - 20) && (check_car_s - car_s < 100)) {
					if ((d < 4) && (d > 0)) {
						lane_free[0] = false;
					}
					else if ((d >= 4) && (d < 8)) {
			...
			*/
			...
			if ((too_close) || ((move_two_lanes) && ref_vel > 35)) {
				ref_vel -= .25;
			}
			...
			if (lane_free[1]) {
				lane = 1;
			} else if ((too_close) && (lane_changing == false)) {
				lane_changing = true;
				if (lane == 0 ) {
					if (lane_available[1] == true) {
						lane++ ;
			...
			if (move_two_lanes) {
				for (int a = 0; a < mylist.size(); a++) {
					if ((mylist[a] < 8 ) && (mylist[a] > 4 )) {
						pos_car_two_lanes = car_s - pos_car[a];
						pos_car_tw.push_back(pos_car_twolanes);
					}
					for (int b = 0; b < pos_car_tw.size(); b ++) {
						if (pos_car_tw[b] < 10) {
						}
					}
				}
